(function(){
  ...
  function clusterFor(area){
    ...
    const rules=[[/تجمع|رحاب|قاهرهجديده/,0],[/نصر|مصرالجديده|هليوبوليس|روكسي|نزهه|جسربسويس|زيتون/,1],[/عبور|شروق|مدينتي|بدر/,2],[/معادي|معادى|بساتين|دارالسلام|مصرالقديمه|سيدهنبين|مقطم/,3],[/حلوان|مايو|معصره|تبين/,4],[/اكتوبر|زايد|اهرام/,5],[/جيزه|فيصل|هرم|عمرانيه|ابوالنمرس/,6],[/شبرا|رمسيس|شعريه|بولاق|زاويه|قبه|اميريه|عينشمس|مطريه|نخل|خصوص|قليوب|خانكه/,7]];
    for(const [re,i] of rules)if(re.test(n))return i;return -1;
  }
  function ensureClusters(){
    const out=CLUSTERS.map(c=>({name:c.name,areas:c.areas.slice()}));
    const known=new Set(out.flatMap(c=>c.areas.map(norm)));
    (data||[]).forEach(r=>{if(!r.area)return;const i=clusterFor(r.area);if(i<0&&!known.has(norm(r.area))){known.add(norm(r.area));out.push({name:'Route '+(out.length+1)+' — '+r.area,areas:[r.area]});}});
    return out;
  }
  function rowsByCluster(){
    const cs=ensureClusters(),out=cs.map(()=>[]);
    (data||[]).forEach(r=>{let i=clusterFor(r.area);if(i<0)i=cs.findIndex(c=>c.areas.some(a=>norm(a)===norm(r.area)));if(i>=0)out[i].push(r);});
    return out.map((rs,i)=>[i,rs]).filter(([,rs])=>rs.length);
  }
  function addCourierButton(){
    if(document.getElementById('courierSheet'))return;
    const bar=document.querySelector('.bar');if(!bar)return;
    const b=document.createElement('button');b.id='courierSheet';b.className='btn secondary';b.textContent='🚚 شيت المندوب';b.onclick=downloadCourierSheet;bar.insertBefore(b,document.getElementById('refresh'));
  }
  function downloadCourierSheet(){
    const cs=ensureClusters(),rows=[];
    rowsByCluster().forEach(([i,rs])=>{const route=cs[i]?.name||('Route '+(i+1)),seen=new Set();rs.forEach(r=>{const key=(r.order||'')+'|'+(r.area||'');if(r.order&&!seen.has(key)){seen.add(key);rows.push({'Route':route,'Zone / Area':r.area||'غير مصنف','Order No.':r.order,'Courier Name':''});}});});
    if(!rows.length){alert('لا توجد Orders متاحة حاليًا.');return;}
    const ws=XLSX.utils.json_to_sheet(rows);ws['!cols']=[{wch:30},{wch:25},{wch:20},{wch:25}];
    const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'Courier Sheet');
    XLSX.writeFile(wb,'Courier_Sheet.xlsx');
  }
  function renderGeo(){
    const cs=ensureClusters(),zr=rowsByCluster(),am=new Map();
    (data||[]).forEach(r=>{if(!am.has(r.area))am.set(r.area,new Set());if(r.order)am.get(r.area).add(r.order);});
    ...
    $('ko').textContent=new Set((data||[]).map(r=>r.order).filter(Boolean)).size;
    $('ks').textContent=new Set((data||[]).map(r=>r.sku).filter(Boolean)).size;
    $('kq').textContent=(data||[]).reduce((s,r)=>s+r.qty,0);
    $('kz').textContent=groups.length;
    $('kv').textContent=allocate((data||[]).reduce((s,r)=>s+r.cbm,0)).vehicles||0;
    ...
    (data||[]).forEach(r=>{const i=clusterFor(r.area);r.route=i>=0?cs[i].name:'Route — '+r.area;});
    renderDetails();
    drawGeoMap(groups,cs);
    if($('status')&&data.length)$('status').textContent='تم تحميل الداتا وتطبيق Geographic Clustering ✓';
  }
  function drawGeoMap(groups,cs){
    try{
      if(!map){map=L.map('map').setView([30.03,31.25],9);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(map);}
      layer.forEach(x=>x.remove());layer=[];
      const all=[L.latLng(DEPOTS.W100.lat,DEPOTS.W100.lng)];layer.push(L.marker(all[0]).addTo(map).bindPopup('<b>W100 · Warehouse 1</b><br>Start Depot'));
      groups.forEach(c=>{const pts=[L.latLng(DEPOTS.W100.lat,DEPOTS.W100.lng)];const center=CENTERS[c.i]||[30.03,31.25];uniqueOrders(c.rs).forEach((r,j)=>{const lat=r.lat>20&&r.lat<35?r.lat:center[0],lng=r.lng>20&&r.lng<40?r.lng:center[1],p=L.latLng(lat,lng);pts.push(p);all.push(p);layer.push(L.marker(p).addTo(map).bindPopup(`<b>${E(cs[c.i].name)}</b><br>Stop: ${j+1}<br>Order: ${E(r.order)}<br>Customer: ${E(r.customer)}<br>Area: ${E(r.area)}<br>Phone: ${E(r.phone)}`));});if(pts.length>1)layer.push(L.polyline(pts,{weight:4,color:COLORS[c.i]||'#ff7900'}).addTo(map));});
      if(all.length>1)map.fitBounds(L.latLngBounds(all),{padding:[20,20]});setTimeout(()=>map.invalidateSize(),150);
    }catch(e){console.error('Geo map failed',e);}
  }
  function install(){
    try{
      if(typeof render!=='function'||typeof loadBuf!=='function'||typeof XLSX==='undefined'||typeof L==='undefined')return setTimeout(install,250);
      const originalLoadBuf=loadBuf;
      render=renderGeo;
      loadBuf=async function(buf){await originalLoadBuf(buf);renderGeo();};
      addCourierButton();
      if(data&&data.length)renderGeo();
    }catch(e){console.error('Geo clustering install failed',e);}
  }
  install();
})();